...
func New() *Generator {
	...
	g.LogPrefix = "graphqlc-gen-echo"
	return g
}

func (g *Generator) fileRenameFn(name string) string {
	if ext := path.Ext(name); ext == ".graphql" {
		name = name[:len(name)-len(ext)]
	}
	suffix := ".echo.graphql"
	if v, ok := g.Param["suffix"]; ok {
		suffix = v
	}
	name += suffix
